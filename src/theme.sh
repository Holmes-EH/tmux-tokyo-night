...
if [ "$theme_disable_plugins" -ne 1 ]; then
  ...
  for plugin in "${plugins[@]}"; do
    ...
    elif [ "$plugin" == "tmux-cpu" ]; then

      separator_icon_start="#[bg=default]#{cpu_fg_color}${right_separator}#[none]"
      separator_icon_end="#[fg=${PALLETE[blue7]}]#{cpu_bg_color}${right_separator}#[none]"
      plugin_icon_output="${separator_icon_start}${separator_icon_end}"

      plugin_icon="#[bg=${PALLETE[blue7]}]#{cpu_fg_color}  "
      plugin_output="#[fg=${PALLETE[white]},bg=${PALLETE[blue7]}] #{cpu_percentage}"
      separator_end="#[fg=${PALLETE[blue7]},bg=default]${right_separator_inverse}#[none]"

      plugin_output_string="${plugin_icon_output}${plugin_icon}${plugin_output} ${separator_end}"
