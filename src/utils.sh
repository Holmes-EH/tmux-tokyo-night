...
function generate_active_window_string() {
  ...
  left_separator=$(get_tmux_option "@theme_left_separator" "")
  zoomed_window_icon=$(get_tmux_option "@theme_plugin_zoomed_window_icon" " ")
  pane_synchronized_icon=$(get_tmux_option "@theme_plugin_pane_synchronized_icon" "󰓦")
  transparent=$(get_tmux_option "@theme_transparent_status_bar" "false")
  ...
  pane_synchronized_string="#[bg=${PALLETE[magenta2]}] ${pane_synchronized_icon}#[fg=${PALLETE[magenta2]}]#[bg=default]${left_separator}"

  echo "${separator_start}#[fg=${PALLETE[white]}]#I${separator_internal}#[fg=${PALLETE[white]}] #{?window_zoomed_flag,$zoomed_window_icon,$active_window_icon}${active_window_title}#{?pane_synchronized,${pane_synchronized_string},${separator_end}}#[none]"
}
